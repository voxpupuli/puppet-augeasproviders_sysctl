  ...
  module SysctlValueSync
    def insync?(is)
      ...
      if provider.valid_resource?(resource[:name])
        ...
        _is_insync = equal(should, is) if _is_insync && (resource[:persist] == :true)
      else
    ...
    def change_to_s(current, new)
      ...
      if equal(current, new)
        ...
        return "changed configuration value from '#{current}' to '#{new}' and live value from '#{@live_value}' to '#{new}'" if resource[:persist] == :true
